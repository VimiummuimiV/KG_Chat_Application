...
function createUserListUI(users) {
  ...
  return sortedUsers.map(user => {
    let avatarElement;
    if (user.avatar) {
      // Transform the avatar URL to use the big version by appending "_big" before ".png"
      const bigAvatarUrl = user.avatar.replace(".png", "_big.png");
      avatarElement = `<img class="user-avatar" src="${BASE_URL}${bigAvatarUrl}" alt="${user.login}'s avatar">`;
    } else {
      const emoji = getRandomEmojiAvatar();
      avatarElement = `<span class="user-avatar">${emoji}</span>`;
    }
...
class UserManager {
  ...
  updatePresence(xmlResponse) {
    console.log('📥 Received body response:', xmlResponse);

    const parser = new DOMParser();
    const doc = parser.parseFromString(xmlResponse, "text/xml");
    const presences = doc.getElementsByTagName("presence");
    console.log(`🔍 Found ${presences.length} presence stanzas`);
    ...
    for (let i = 0; i < presences.length; i++) {
      ...
      // Extract user data (store original avatar URL as received)
      ...
      const user = {
        jid: from,
        login,
        avatar, // Original avatar URL is stored here
        color: background,
        role,
        game
      };
  ...
  startPresencePolling() {
    setInterval(async () => {
      try {
        ...
        // Only process and log if presence updates were received
        if (response.includes('<presence')) {
          console.log('📥 Presence information received in polling response');
          this.userManager.updatePresence(response);
        } else {
          console.log('📡 Empty polling response (keeping connection alive)');
        }
      } catch (error) {
